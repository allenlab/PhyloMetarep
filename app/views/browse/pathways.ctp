...
if($mode === KEGG_PATHWAYS) {
	...
	$searchQuery = $session->read("$function.browse.pathwayQuery");
}
else if($mode === KEGG_PATHWAYS_KO) {
	$function = 'keggPathwaysKo';
	$dialog->browseKeggPathways('dialog');	
	$selectedNode = $session->read("$function.browse.selectedNode");
	$searchQuery = "kegg_tree:$selectedNode";
}
...
$filterQuery = ($filter === '*:*' ? "$searchQuery" : "($searchQuery AND $filter)");
