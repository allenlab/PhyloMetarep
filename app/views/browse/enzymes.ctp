<!----------------------------------------------------------
  ...
  File: enzymes.ctp
  Description: View page to browse the Enzyme Classification 
...
if($session->check('enzymes.browse.query')) {
	$filter = $session->read('enzymes.browse.query');
}
else {
	$filter = '*:*';
}
...
$filterQuery = ($filter === '*:*' ? "ec_id:$selectedNode" : "(ec_id:$selectedNode AND $filter)");
$facetFields = $session->read("$mode.browse.facetFields");
		...
		<div id="browse-left-panel">		
			<div id="browse-search-panel">			
				<fieldset>
					...
					<?php echo $ajax->submit('Filter', array('url'=> array('controller'=>'browse', 'action'=>'filter',$dataset,'enzymes'),'update' => 'Browse', 'loading' => 'Element.show(\'spinner\')', 'complete' => 'Element.hide(\'spinner\'); Effect.Appear(\'browse-main-panel\',{ duration: 1.5 })', 'before' => 'Element.hide(\'browse-main-panel\');'));?>
					<?php echo $form->end();?>
				</fieldset>
			</div>		
			<div id="browse-tree-panel">			
				<fieldset>
				<legend>Browse Enzymes</legend>				
					<?php 	
					if($numHits == 0) {
