<!----------------------------------------------------------
  ...
  taxonomic BLAST assignment stored in the blast_tree field.
...
if($session->check('blastTaxonomy.browse.query')) {
	$filter = $session->read('blastTaxonomy.browse.query');
}
else {
	$filter = '*:*';
}
...
$filterQuery = ($filter === '*:*' ? "blast_tree:$selectedNode" : "(blast_tree:$selectedNode AND $filter)");
...
<div id="Browse">
	...
	<h2><?php __("Browse Taxonomy (Best Hit)");?><span class="selected_library"><?php echo "$dataset ($projectName)"; ?></span>	
	...
	<div id="browse-main-panel">
		...
		<div id="browse-left-panel">		
			<div id="browse-search-panel">			
				<fieldset>
					...
					<?php #echo $ajax->submit('Update', array('controller'=> 'browse','action'=>'filter'), array('update' => 'Browse', 'loading' => 'Element.show(\'spinner\')', 'complete' => 'Element.hide(\'spinner\'); Effect.Appear(\'browse-main-panel\',{ duration: 1.5 })', 'before' => 'Element.hide(\'browse-main-panel\')'));?>				
					<?php echo $ajax->submit('Filter', array('url'=> array('controller'=>'browse', 'action'=>'filter',$dataset,'blastTaxonomy'),'update' => 'Browse', 'loading' => 'Element.show(\'spinner\')', 'complete' => 'Element.hide(\'spinner\'); Effect.Appear(\'browse-main-panel\',{ duration: 1.5 })', 'before' => 'Element.hide(\'browse-main-panel\');'));?>
					<?php echo $form->end();?>
				</fieldset>
			</div>				
		...
		<div id="browse-right-panel">
			...
			<div id="browse-facet-pie-panel">
				<?php echo $html->div('browse-download-facets', $html->link($html->image("download-medium.png"), array('controller'=> 'browse','action'=>'dowloadFacets',$dataset,$taxon,$mode,$numHits,urlencode($filter)),array('escape' => false)));?>	
				<?php  
					echo $facet->topTenPieCharts($facets,$facetFields,$numHits,"700x200","300x150");
					?>
			</div>
		</div>
		<?php endif;?>
