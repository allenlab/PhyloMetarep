...
foreach ($projects as $project) {	
	...
	echo("<ul id=\"tile_001\" class=\"jGlide_001_tiles\" title=\"Quick Navigation\" alt=\"Select investigator from list of ".count($projects)." projects \">");	                
	echo("<li rel=\"project_id_{$project['Project']['id']}\">".$project['Project']['name']." (". count($project['Library'])." Transcriptomes)</li>");				               	
	echo('</ul>');	
	
	//build population entries
	foreach ($project['Population'] as $population) {	
		echo("<ul id=\"project_id_{$project['Project']['id']}\" class=\"jGlide_001_tiles\" title=\" {$project['Project']['name']} \" alt=\"Select transcriptome from list of ".(count($project['Population']) + count($project['Library']))." datatsets \">");                			
	...
	foreach ($project['Library'] as $library) {	
		...
		echo("<li>".$html->link(__('Browse Taxonomy (Best Hit)', true), array('controller'=> 'browse', 'action'=>'blastTaxonomy',$library['name']))."</li>");
