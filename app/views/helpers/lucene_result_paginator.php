		...
		foreach ($hits as $hit ) {	
			...
			foreach($resultFields as $fieldId => $fieldName) {
				...
				if(!is_null(ClassRegistry::init("Phylodb.Protein")) && $fieldId === 'peptide_id' && !empty($hit->{'com_name_src'})) {	
					...
					$html .=  "<td>{$hit->{$fieldId}}<BR>".$this->Html->link('[phylodb homolog]', array('plugin' => 'phylodb','controller'=> 'phylodb', 'action'=>'protein',$hit->{'com_name_src'}),array('target'=>'_blank')); 
					$html .=  $this->Html->link('[feature]', array('plugin' => null,'controller'=> 'peptides', 'action'=>'drawFeatures',$dataset,42,$hit->{$fieldId},'Search'),array('target'=>'_blank'));
					
					if(file_exists(SEQUENCE_STORE_PATH."/42/$dataset/tree/{$hit->{$fieldId}}.pdf")) {						
						$html .=  $this->Html->link('[tree]', array('plugin' => null,'controller'=> 'peptides', 'action'=>'downloadApisTree',$dataset,42,$hit->{$fieldId}),array('target'=>'_blank'))."</td>"; 
					}
				} 
				else {					
