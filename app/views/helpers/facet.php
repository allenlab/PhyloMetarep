...
class FacetHelper extends AppHelper {
	...
	function printFacet($facet,$results,$hits,$facetField=null,$dataset=null,$filterQuery=null) {
		$html = null;
...
//				}
//			}		
			if($facetField=='blast_species') {
				$id  = "\"$id\"";
			}		

			
			//add linkd if facet field is set
			if($facetField) {
				$query = (empty($filterQuery) || $filterQuery === '*:*' ? "$facetField:$id" : "$facetField:$id AND $filterQuery");
				$query = preg_replace("/:/","@",$query);		
				$linkOptions = array('plugin' => null,'controller'=>'search','action'=>'link','index',$query,$dataset);
				$imgLink = $this->Html->link($this->Html->image("link-small2.png",array("title" => "Search for $id" )),$linkOptions ,array('escape' => false));						
				$link = $this->Html->link($class,$linkOptions,array("title" => "Search for $id" ));
				$html.= "<li><B>$link</B><BR>$imgLink  ($perc%) ($count hits)</li>";			
			}
			else {
				$html.= "<li><B>$class</B><BR>($perc%) ($count hits)</li>";
			}
	...
	function topTenList($facets,$facetFields,$numHits,$dataset=null,$filterQuery=null) {
						...
						<table cellpadding=\"0\" cellspacing=\"10\" border =0 valign=\"TOP\" style=\"border-style:none !important;\">
							...
							foreach($facetFields as $facetField => $facetName) { 
								...
								$html .= "<td valign=\"TOP\" $style>".
											$this->printFacet($facetName,$facets->facet_fields->{$facetField},$numHits,$facetField,$dataset,$filterQuery).
										 "</td>";
		...
		return "<div class=\"top 10 facets form\">
				<fieldset>
					<legend>Top Ten Functional Pie Charts</legend>
						<table cellpadding=\"0\" cellspacing=\"0\" style=\"border-style:none !important;\" >
							<tr>
								<td valign=\"TOP\">".$this->pieChart('Species (Best Hit)',$facets->facet_fields->blast_species,$numHits,$sizeLarge)
			...
			<fieldset>
				<legend>Top Ten Meta-Information</legend>
					<table cellpadding=\"0\" cellspacing=\"10\" valign=\"TOP\" style=\"border-style:none !important;\" >
						<tr>
							<td valign=\"TOP\">".$this->printFacet('Investigator',$facets['project'],$numHits)."</td>
							<td valign=\"TOP\">".$this->printFacet('Sample Habitat',$facets['habitat'],$numHits)."</td>
							<td valign=\"TOP\">".$this->printFacet('Sample Organism',$facets['filter'],$numHits)."</td>
				...
				<fieldset>
					<legend>Top Ten Meta-Information Pie Charts</legend>
						<table cellpadding=\"0\" cellspacing=\"0\" style=\"border-style:none !important;\">
							<tr>
								<td valign=\"TOP\">"
										.$this->pieChart('Investigator',$facets['project'],$numHits,$sizeLarge)
										.$this->pieChart('Sample Habitat',$facets['habitat'],$numHits,$sizeLarge)."</td>
								<td style=\"border-right:none\">"	.$this->pieChart('Sample Organism',$facets['filter'],$numHits,$sizeSmall).
