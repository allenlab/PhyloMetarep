...
class Taxonomy extends AppModel {
	...
	public function getTreeQueryByName($name,$field,$rank = '') {		
		$query = "SELECT count(*) as hits,replace(GROUP_CONCAT(DISTINCT CONCAT('$field:',taxon_id) separator ' OR '),'-','\\\-') as query,  GROUP_CONCAT(DISTINCT concat(taxon_id,' ',name,' ',rank) separator '@') as suggestions FROM taxonomy WHERE name like '%$name%'";
		if(!empty($rank)) {
			$query.=" AND rank='$rank'"; 
		}		
