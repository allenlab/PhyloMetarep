...
class PhylodbPaginatorHelper extends AppHelper {
	...
	function data($dataset,$hits,$page,$numHits,$limit,$sessionQueryId,$resultFields) {
		...
		foreach ($hits as $hit ) {	
			...
			foreach($resultFields as $fieldId => $fieldName) {
//				
//				if(!is_null(ClassRegistry::init("Phylodb.Protein")) && $fieldId === 'com_name_src') {
//					
//					$html .=  "<td>".$this->Html->link($hit->{$fieldId}, array('plugin' => 'phylodb','controller'=> 'phylodb', 'action'=>'protein',$hit->{$fieldId}),array('target'=>'_blank'))."</td>"; 
//				} 
//				else {
