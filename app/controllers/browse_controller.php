...
class BrowseController extends AppController {
	...
	var $uses 		= array();	
	var $components = array('Session','RequestHandler','Solr','Format','Download','Color');
	...
	function blastTaxonomy($dataset,$expandTaxon=1,$query='*:*') {
		...
		#$this->Session->delete(BLAST_TAXONOMY.'.browse.tree');
		$function = __FUNCTION__;		
		
		$this->loadModel('Project');
		$this->loadModel('Taxonomy');
		$this->pageTitle = 'Browse Taxonomy (Best Hit)';
		...
		foreach($taxaChildren as $taxon) {
			...
			$solrAguments = array('fq'=>"blast_tree:$searchTaxonId");		
		...
		$solrArguments = array(	
						'fq'=> "blast_tree:$searchExpandTaxon",
						'facet' => 'true',
		...
		$this->Session->write(BLAST_TAXONOMY.'.browse.selectedNode',$expandTaxon);	
		...
		if($expandTaxon==1) {
			$this->setAction('blastTaxonomy',$dataset,131567,$query);
			return $this->setAction('blastTaxonomy',$dataset,2759,$query);
		}
	}
	...
	*/
	...
	function apisTaxonomy($dataset,$expandTaxon=1,$query='*:*') {
		...
		$this->Session->write(APIS_TAXONOMY.'.browse.selectedNode',$expandTaxon);	
		...
		## set default behaviour
		if($expandTaxon==1) {
			$this->setAction('apisTaxonomy',$dataset,131567,$query);
			return $this->setAction('apisTaxonomy',$dataset,2759,$query);
		}		
	}
	...
	function enzymes($dataset,$expandTaxon='root',$query = '*:*') {		
		...
		$this->Session->write(ENZYMES.'.browse.selectedNode',$expandTaxon);	
...
//		}		
		...
		$this->Session->write(GENE_ONTOLOGY.'.browse.selectedNode',$expandTaxon);		
		...
		## set default behaviour
		if($expandTaxon==='root') {
			$this->setAction('geneOntology',$dataset,8150,$query);
		}			
	}
	 ...
	 * @access public
	 */
	public function pathways($dataset,$parentId,$query,$pathwayModel) {
		...
		$pathwayQuery =  $this->Pathway->getQuery($parentId,$pathwayModel);
		...
		$this->Session->write("$function.browse.selectedNode",$parentId);
		$this->Session->write("$function.browse.pathwayQuery",$pathwayQuery);
		...
		## set default behaviour
		if($parentId == 1) {	
			return $this->setAction($function,$dataset,2,$query);
		}	
