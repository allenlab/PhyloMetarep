...
class PopulationsController extends AppController {
	...
	function add($projectId = 0) {
		...
		// pull datasets
		$datasetRes = $this->Population->Library->find('all',array('conditions'=>array('project_id'=>$projectId)));
		
		$datasets = array();
		
		//add description to dataset label
		foreach($datasetRes as $dataset) {
			if(!empty($dataset['Library']['description'])) {
				$datasets[$dataset['Library']['id']] = "{$dataset['Library']['name']} ({$dataset['Library']['description']})";
			}
			else {
				$datasets[$dataset['Library']['id']] = "{$dataset['Library']['name']}";
			}
		}			
