...
class SearchController extends AppController {
	...
	var $uses 			= array();
	var $components 	= array('Session','RequestHandler','Solr','Format','Download','Blast');

	var $searchFields 	= array(1 => 'Lucene Query',

								'Search By ID' => array( 
											'peptide_id'=>'Peptide ID',											
											'blast_tree'=>'PhyloDB Taxonomy (Best Hit) ID',	
								...
								'Search By Name' => array( 
									...
									'blast_tree_name'=>'PhyloDB Taxonomy (Best Hit) Name',	
									'blast_species_name' =>'PhyloDB Species (Best Hit) Name',
	...
	var $facetFields = array(
								'blast_species'=>'Species (Best Hit)',
	...
	var $luceneFields = array('peptide_id','com_name_txt','com_name_src','go_id','go_tree',
							  ...
							  'cluster_id','filter','kegg_tree','ko_id','weight','scaf_id');
	...
	function index($dataset,$page=1,$sessionQueryId=null) {
		...
		if($optionalDatatypes['viral'] || $optionalDatatypes['clusters'] || $optionalDatatypes['apis'] || $optionalDatatypes['filter']  || $optionalDatatypes['ko'] || $optionalDatatypes['sequence'])  {
			...
			if($optionalDatatypes['apis']) {
				$this->searchFields['Search By ID']['apis_tree']= 'PhyloDB Taxonomy (Apis) ID';
				$this->searchFields['Search By Name']['apis_tree_name']= 'PhyloDB Taxonomy (Apis) Name';
				$this->searchFields['Search By Name']['apis_species_name']= 'PhyloDB Species (Apis) Name';
			}
		...
		if ($query != "*:*" || ($searchAllResults = Cache::read($currentUserId.'searchAllResults')) === false) {	
			...
			foreach($datasets as &$dataset) {				
				...
				if($numHits > 0 ) {
					...
					$libraryMetadata = $this->Library->find('all', array('fields'=>array('sample_habitat','sample_organism','sample_longitude','sample_latitude','sample_depth'),'conditions' => array('Library.name' => $dataset['name'])));
					
					$habitat 	= $libraryMetadata[0]['Library']['sample_habitat'];
					$filter 	= $libraryMetadata[0]['Library']['sample_organism'];
	 ...
	 * @access public
	 */
	public function dowloadData($dataset,$numHits,$sessionQueryId,$idsOnly) {
		$this->autoRender=false;
	...
	public function dowloadSequences($dataset,$projectId,$numHits,$sessionQueryId) {
		...
		// static ID to handle the join file store; all phylodb is stored in project 42
		$projectId = 42;
		...
		//specify fasta file
		$fastaFileName 	= uniqid('jcvi_phylogenomics_report_').'.fasta';
		...
		foreach($datasets as $dataset) {			
			...
			for($i=0;$i<$numHits+$batchSize;$i+=$batchSize) {
				...
				foreach ( $documents as $document ) {
					## handle NCGR peptides					
					$peptideId = str_replace('|','@',$document->peptide_id);
					fwrite($fh, $peptideId."\n");
				}
			...
			fclose($fh);
			#debug(FASTACMD_PATH." -d ".SEQUENCE_STORE_PATH."/$projectId/$dataset/$dataset -i $idFilePath >> $fastaFilePath");
		...
		exec(LINUX_BINARY_PATH."/sed -i 's/@/|/' $fastaFilePath");
		
		$this->Download->textFile($fastaFileName,$fastaFilePath);		
	}	
			...
			//handle non-Lucene queries
				...
				switch($field) {
					...
					case "apis_tree_name":
						...
						$searchByNameResults = $this->TaxonomyApis->getTreeQueryByName($query,'apis_tree');
						break;
					case "apis_species_name":
						$this->loadModel('TaxonomyApis');
						$searchByNameResults = $this->TaxonomyApis->getTreeQueryByName($query,'apis_tree','species');
						break;
					case "hmm_name":
						$this->loadModel('Hmm');
					...
					case "seq_id05":
						$this->loadModel('Project');							
						$searchByNameResults = $this->Blast->getSubjectIdQueryBySequence($dataset,$query,'1E-5');
						break;	
					case "seq_id10":
						$this->loadModel('Project');							
						$searchByNameResults = $this->Blast->getSubjectIdQueryBySequence($dataset,$query,'1E-10');
						break;	
					case "seq_id50":
						$this->loadModel('Project');							
						$searchByNameResults = $this->Blast->getSubjectIdQueryBySequence($dataset,$query,'1E-50');
						break;																		
					case "cluster_name":
