...
//		 		}
	 ...
	 * If a population is supplied it executes a distributes search across
	 * population datasets and returns aggregated results.
	 *
	 * @param String $dataset dataset or dataset population
	 * @param String $query lucene query string http://lucene.apache.org/java/2_4_0/queryparsersyntax.html
	...
	## function for weighted facets
	public function weightedFacetSearch($dataset,$query,$params) {
		...
		$facetsDetailed = $result->stats->stats_fields->weight->facets->peptide_id;
		unset($result);
		$result =array();
		
		foreach($facetsDetailed as $key => $value) {
