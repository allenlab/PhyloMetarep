...
class FormatComponent extends Object {
	...
	public function facetToDownloadString($heading,$facets,$numHits) {			
			$content ="$heading\tAbsolute Count\tRelative Count (Proportion)\n";
			$content .="----------------------------------------------------------\n";
	...
	public function pathwayToDownloadString($facetName,$data,$numHits) {
		...
		foreach ($data as $level2=>$pathways) {	
			...
			$content .= "Pathway\tAbsolute Count\tRelative Count (Proportion)\tPathway Enzymes\t#Found Enzymes\t%Found Enzymes\n";
	...
	public function infoString($title,$dataset,$query,$minCount,$numHits='',$node ='',$evalue ='',$sequence=''){
		$content  ="#------------------------------------------------------------------------------------------------------\n";
	...
	public function facetMetaInformationListToDownloadString($title,$facets,$query,$numHits,$numDatasets,$node = ''){		
		...
		$content.=$this->facetToDownloadString('Investigator',$facets['project'],$numHits);	
		$content.=$this->facetToDownloadString('Sample Habitat',$facets['habitat'],$numHits);		
		$content.=$this->facetToDownloadString('Sample Organism',$facets['filter'],$numHits);		
	...
	public function blastAnnotationToDownloadString($title,$facets,$query,$numHits,$numDatasets,$node = '') {
...
//			$content.= $this->printMultiValue($row->hmm_id)."\n";
	...
	public function edgeResultsToDownloadString($counts,$selectedDatasets,$maxPvalue) {
		...
		$content .= "Log Concentration\t";
		$content .= "Log Fold Change\t";
		$content .= "\tDispersion\t";
		$content .= "\tP-Value (Bonferroni)\t";
		$content .= "\tP-Value (Bonferroni)\t";
		$content .= "\tQ-Value (FDR)\t";
